 ...
 * @brief Threat Data Fetch and Encryption (Legacy)
 * 
 * MIT License
 ...
 * 
 * @deprecated Legacy implementation; see src/data-ingestion.cpp for current version
 * 
 * @details
 * Implements the data ingestion pipeline:
 * 1. HTTP GET to Flask API to fetch threat records
 * 2. CKKS encryption of threat scores
 * 3. Return encrypted data for clustering
 * 
 * The main() function demonstrates complete workflow: init context,
 * generate keys, fetch data, encrypt, and count successful encryptions.
 * 
...
/**
 * @brief Callback for libcurl to accumulate HTTP response
 */
...
/**
 * @brief Fetch threat records from Flask API
 * 
...
/**
 * @brief Encrypt threat scores from JSON records
 * 
 * @param cc Initialized CKKS context
 * @param keyPair Public/secret key pair
 * @param threat_data JSON array of threat records
 * 
 * @return Vector of CKKS ciphertexts (one per record)
 * 
 * Extracts threat_score from each record and encrypts under public key
 */
